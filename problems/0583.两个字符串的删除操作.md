...
int minDistance(char* word1, char* word2) {
    ...
    // 创建动态规划数组，dp[i][j]表示word1的前i个字符与word2的前j个字符的最小编辑距离
    int dp[word1Len + 1][word2Len + 1];
    // 初始化动态规划数组的第一列，如果将word1变为空字符串，则需要对所有word2的字符进行插入操作
    for (int i = 0; i <= word1Len; ++i) {
        dp[i][0] = 1;
    }
    // 初始化动态规划数组的第一行，如果将word2变为空字符串，则需要对所有word1的字符进行删除操作
    for (int j = 1; j <= word2Len; ++j) {
        dp[0][j] = 0;
    }
    for (int i = 1; i <= word1Len; ++i) {
        for (int j = 1; j <= word2Len; ++j) {
            // 如果当前字符相等，则最小编辑距离不会改变
            if(word1[i - 1] == word2[j - 1]){
                dp[i][j] = dp[i - 1][j - 1];
            } else{
                // 如果当前字符不相等，则考虑删除操作中的最小代价
